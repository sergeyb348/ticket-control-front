...
const UsherEvent = observer(() =>{
    ...
    const getUsher = async () =>{
        console.log(params.id)
        let usher = await AuthServices.getUsher(params.usherId)
        setEmail(usher.email)
        setFirstName(usher.firstName)
        setLastName(usher.surname)
        setSurname(usher.surname)
        console.log(usher)
    }

    const changeUsher = async () =>{
        try {
    ...
    const deletUsher = async () =>{
        Swal.fire({
            ...
            if(!result.isConfirmed)
                return
                console.log(params.id)
            const res = await AuthServices.deleteUsher(params.id)
            console.log(res)
    ...
    const removeFromEvent = async (usher) =>{
        Swal.fire({
            title: 'Вы уверены?',
            icon: 'warning',
            showCancelButton: true,
            ...
            const res = await AuthServices.removeFromEvent(params.eventId, params.usherId)
            console.log(res)
            ...
            <Form className="container">
                ...
                <div class="mt-4 mb-4 row">
                    ...
                    <Button 
                        ...
                        <Button 
                        className="btn btn-danger fs-5 me-2 mt-4" 
                        onClick={deletUsher}>
                            Удалить
                        </Button>
                        <Button 
